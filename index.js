    ...
    if (err) {
      ...
      for (var i = 0; i < data.instanceSnapshots.length; i++) {
        ...
        if (backupFromInstance == instanceName) {
          ...
          if (
            backupDaysTillNow > backupDaysMax &&
            backupDaysTillNow <= backupWeeksMax * 7 &&
            backupDate.getDay() == 0
          ) {
            saveBackup = true;
          }
          ...
          if (
            backupDaysTillNow > backupWeeksMax * 7 &&
            backupDaysTillNow <= backupMonthsMax * 30 &&
            backupDate.getDate() < 8 &&
            backupDate.getDay() == 0
          ) {
            saveBackup = true;
          }
          ...
          if (saveBackup) {
            ...
            console.log(
              `kept ${backupDate.getDate()} ${
                data.instanceSnapshots[i].createdAt
              }  ${data.instanceSnapshots[i].name}`
            );
          } else {
          ...
          console.log(
            "Will ignore this backup because it belongs to an instance other than " +
              instanceName
          );
        }
      }
